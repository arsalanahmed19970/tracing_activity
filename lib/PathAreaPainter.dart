...
class PathAreaPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (showCompletedPath && currentDotIndex > 0) {
      final completedPoints = _getCompletedPathPoints(allPathPoints, currentDotIndex, nonNuktaDots);
      _drawPathArea(canvas, completedPoints, 
    ...
    if (currentDotIndex < nonNuktaDots.length - 1) {
      final remainingPoints = _getRemainingPathPoints(allPathPoints, currentDotIndex, nonNuktaDots);
      ...
      _drawPathArea(canvas, remainingPoints, 
          Colors.blue.withOpacity(pulseOpacity), 
          Colors.blue.withOpacity(borderOpacity));
    }
  }

  List<Offset> _createSimplePath(List<TracingPoint> dots, List<Offset> pathOffsets) {
  ...
  List<Offset> _getCompletedPathPoints(List<Offset> allPoints, int currentDotIndex, List<TracingPoint> dots) {
    ...
    for (final point in allPoints) {
      if (_distanceFromStart(point, dots[0].position) <= _distanceFromStart(currentDot, dots[0].position)) {
  ...
  List<Offset> _getRemainingPathPoints(List<Offset> allPoints, int currentDotIndex, List<TracingPoint> dots) {
  ...
  void _drawPathArea(Canvas canvas, List<Offset> points, Color fillColor, Color borderColor) {
